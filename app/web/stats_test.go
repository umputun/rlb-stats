...
func TestStats(t *testing.T) {
	...
	for i, result := range results {
		assert.EqualValues(t, result.topFiles, getTop("files", candles, i), "candle match with expected output")
		assert.EqualValues(t, result.topNodes, getTop("nodes", candles, i), "candle match with expected output")
	}

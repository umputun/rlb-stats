...
func TestServerAPI(t *testing.T) {
	...
	var testData = []struct {
		...
		{ts: goodServer, url: fmt.Sprintf("/api/candle?from=%v&aggregate=-5m", startTime), responseCode: http.StatusBadRequest,
			result: "{\"error\":\"invalid 'aggregate' field\"}\n"},
		{ts: goodServer, url: fmt.Sprintf("/api/candle?from=%v&aggregate=0s", startTime), responseCode: http.StatusBadRequest,
			result: "{\"error\":\"invalid 'aggregate' field\"}\n"},
		...
		{ts: goodServer, url: "/api/insert", responseCode: http.StatusBadRequest, method: http.MethodPost,
			body:   bytes.NewReader([]byte(`{"from_ip":"127.0.0.1","file_name":"rt_test.mp3","dest":"all","ts":"1970-01-01T01:01:00+01:00"}`)),
			result: "{\"error\":\"dest 'all' is reserved\"}\n"},
		{ts: goodServer, url: "/api/insert", responseCode: http.StatusBadRequest, method: http.MethodPost,
